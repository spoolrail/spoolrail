...
test('accepts repeated synchronization without replacing an existing quorum queue', function (): void {
    ...
    try {
        ...
        $queue = $rabbitMq->management
            ->get('/api/queues/'.rawurlencode($rabbitMq->name).'/'.rawurlencode($queueName))
            ->throw()
            ->json();
        $body = $rabbitMq->management
            ->post(
                '/api/queues/'.rawurlencode($rabbitMq->name).'/'.rawurlencode($queueName).'/get',
                [
                    'count' => 1,
